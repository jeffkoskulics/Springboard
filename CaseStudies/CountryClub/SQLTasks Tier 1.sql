...
SELECT DISTINCT CONCAT(f.name, " ", m.firstname, " ", m.surname) AS membername FROM Members as m
...
SELECT f.name as facility, CONCAT(m.firstname, " ", m.surname) AS membername,
  ...
  JOIN Bookings AS b ON m.memid = b.memid
  JOIN Facilities AS f ON b.facid = f.facid
WHERE Date(b.starttime) = '2012-09-14' AND IF(m.memid = 0, f.guestcost, f.membercost) * b.slots > 30;

/* Q9: This time, produce the same result as in Q8, but using a subquery. */

SELECT * FROM (SELECT f.name as facility, CONCAT(m.firstname, " ", m.surname) AS membername,
  CASE WHEN m.firstname = 'GUEST' THEN f.guestcost * b.slots
  ELSE f.membercost * b.slots END AS cost
  FROM Members AS m
  JOIN Bookings AS b ON m.memid = b.memid
  JOIN Facilities AS f ON b.facid = f.facid
WHERE Date(b.starttime) = '2012-09-14' AND IF(m.memid = 0, f.guestcost, f.membercost) * b.slots > 30) alias;
